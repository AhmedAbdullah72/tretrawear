...
export const CartDrawer = () => {
  ...
  // Listen for custom open-cart event (from FloatingCartButton)
  useEffect(() => {
            ...
            <>
              <div className="flex-1 overflow-y-auto pr-2 min-h-0">
                <div className="space-y-3">
                  {items.map((item) => (
                    <div key={item.variantId} className="flex gap-4 p-3 bg-background rounded-xl border border-border">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-heading font-medium text-sm text-foreground truncate">{item.product.node.title}</h4>
                        <p className="text-xs text-muted-foreground">{item.selectedOptions.map(o => o.value).join(' • ')}</p>
                        <p className="font-heading font-semibold text-sm text-foreground mt-1">{item.price.currencyCode} {parseFloat(item.price.amount).toFixed(2)}</p>
                      </div>
              ...
              <div className="flex-shrink-0 space-y-3 pt-4 border-t border-border">
                ...
                <Button onClick={handleCheckout} className="w-full bg-primary text-primary-foreground hover:bg-primary/90 rounded-xl" size="lg" disabled={items.length === 0 || isLoading || isSyncing}>
